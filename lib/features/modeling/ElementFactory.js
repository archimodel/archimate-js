...
ElementFactory.prototype.create = function(elementType, attrs) {
  ...
  if (elementType === 'connection') {
    ...
    var archimateObject = attrs.businessObject;
    
    if (archimateObject) {
      // get all attrs needed for rendering from archimateObject generated by importer
      logger.log(archimateObject);

      attrs = assign({
        name: archimateObject.relationshipRef && archimateObject.relationshipRef.name,
        label: archimateObject.label,
        id: archimateObject.id
      }, attrs);

      // get style
      if (archimateObject.style) {
        var style = archimateObject.style;
      ...
      if (attrs.type === 'Access') {
        attrs = assign({
          accessType: archimateObject.relationshipRef && archimateObject.relationshipRef.accessType
        }, attrs);
      }

      if (attrs.type === 'Association') {
        attrs = assign({
          isDirected: archimateObject.relationshipRef && archimateObject.relationshipRef.isDirected
        }, attrs);
      }

      if (attrs.type === 'Influence') {
        attrs = assign({
          modifier: archimateObject.relationshipRef && archimateObject.relationshipRef.modifier
        }, attrs);
      }
    } else {
      // this is a new connection to draw on canvas, create core businessObject
      // other attrs (eg. raltionshipRef) will be set in createConnectionBehavior
      // based on attrs directly set on connection element
      if (attrs.type === 'Line') {
        archimateObject = this._archimateFactory.create('archimate:Connection', { type: 'Line' });

      } else {
        archimateObject = this._archimateFactory.create('archimate:Connection', { type: 'Relationship' });

      }
      attrs = assign({
        businessObject: archimateObject,
        id: archimateObject.id,
      }, attrs);
    }
    /*
      ...
      if (attrs.type === 'Line') {
        ...
        archimateObject = this._archimateFactory.createConnection(relationship, { type: 'Relationship' }); //create('archimate:Connection', { relationshipRef: relationship });
    ...
    */
    ...
    logger.log('call baseCreate for connection with attrs:');
    logger.log(attrs);

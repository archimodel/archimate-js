...
import {
  ...
  LAYER_OTHER, LAYER_PHYSICAL, LAYER_TECHNOLOGY
} from '../metamodel/Concept';

export const COLOR_DOMAIN_COMMON = '#D9D3C8',
      COLOR_DOMAIN_RELATIONSHIPS = '#FFFFFF',
      COLOR_LAYER_STRATEGY = '#F5DEAA', // 'rgba(245, 222, 170, 1)'
      ...
      COLOR_LAYER_OTHER = '#FFFFFF';

export const COLOR_LAYER_MAP = new Map([
  [DOMAIN_COMMON, COLOR_DOMAIN_COMMON],
  ['Relationships', COLOR_DOMAIN_RELATIONSHIPS],
  ...
  ['Implementation and Migration', COLOR_LAYER_IMP_MIG],
  [LAYER_MOTIVATION, COLOR_LAYER_MOTIVATION],
  [LAYER_OTHER, COLOR_LAYER_OTHER]
]);

export function getDomainColor(domainName, profile) {
  var domain = profile && (profile.domains || []).find(function(value) {
    return value.name === domainName;
  });

  return (domain && domain.color) || COLOR_LAYER_MAP.get(domainName);
}

...
export default function LabelBehavior(
  ...
  this.postExecute([ 'shape.create', 'connection.create' ], function(event) {
    ...
    logger.log('postExecute[shape.create, connection.create](event)');
    logger.log(event);
  ...
  this.postExecute([ 'no.call' ], function(event) {
    ...
    logger.log('postExecute[no.call](event)');
    logger.log(event);
    ...
    if (!is(tempElementRef, 'archimate:BaseElement')) {
      return;
    }
    ...
    // No name for diObject in the moddle reference, so for this time, no error, but while generate some !!!

    if (!elementRef.name) {
      elementRef.name = archimateFactory.create('archimate:Label', {

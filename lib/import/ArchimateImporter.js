...
ArchimateImporter.prototype.addRoot = function(view, model) {
  ...
  // elementType: 'root'
  rootElement = this._elementFactory.createRoot(attrs);
  ...
  // fix docking in ImportDockingFix  
 ...
 */
ArchimateImporter.prototype.addElement = function(viewElement, parentShape) {
  ...
  logger.log('addElement(viewElement, parentShape):');
  logger.log({ viewElement, parentShape });

  if (isAny(viewElement, [ ARCHIMATE_NODE ])) {

    // var isFrame = is(viewElement.elementRef, 'archimate:Group');

    hidden = parentShape && (parentShape.hidden || parentShape.collapsed);
    ...
    if (viewElement.type (xsi:Type) == 'Label') {
      ...
      // if the elementRef.$type is an ARCHIMATE_ELEMENT so, the type to transfer must be the elementRef.type in order to drawing 
    ...
    */
    ...
    this._canvas.addShape(shape, parentShape, parentIndex);
 ...
 */
 ArchimateImporter.prototype.addConnection = function(viewElement) {
  ...
  if (is(viewElement, ARCHIMATE_CONNECTION) && sourceShape && targetShape) {

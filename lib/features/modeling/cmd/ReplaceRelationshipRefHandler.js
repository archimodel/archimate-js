import { ARCHIMATE_CONNECTION, ARCHIMATE_NODE, ARCHIMATE_RELATIONSHIPS, ARCHIMATE_RELATIONSHIP, RELATIONSHIP_ACCESS, RELATIONSHIP_ASSOCIATION, RELATIONSHIP_INFLUENCE } from '../../../metamodel/Concept';
import { logger } from '../../../util/Logger';
import { canApplyRelationshipMultiplicity } from '../../../util/JunctionUtil';
import { normalizeRelationshipMultiplicity } from '../../../util/MultiplicityUtil';
import { CONNECTION_RELATIONSHIP, is } from '../../../util/ModelUtil';
import { getBaseRelationshipTypeForProfile } from '../../../metamodel/languages';

export default function ReplaceRelationshipRefHandler(eventBus, canvas, archimateFactory, languageProfile) { // commandStack) {
  this._eventBus = eventBus;
  this._canvas = canvas;
  this._archimateFactory = archimateFactory;
  this._languageProfile = languageProfile;
}

ReplaceRelationshipRefHandler.$inject = [
  'eventBus',
  'canvas',
  'archimateFactory',
  'languageProfile'
];

ReplaceRelationshipRefHandler.prototype.execute = function(context) {
  logger.log(context);

  var connection = context.connection,
      newRelationshipRef = context.newRelationshipRef,
      oldRelationshipRef = context.oldRelationshipRef || connection.businessObject.relationshipRef,
      source = connection.source,
      target = connection.target,
      elements = [];

  var eventBus = this._eventBus,
      canvas = this._canvas,
      archimateFactory = this._archimateFactory,
      profile = this._languageProfile && this._languageProfile.get();

  if (connection.businessObject.type === CONNECTION_RELATIONSHIP) {

    var relationshipsNode = canvas._rootElement.modelRef.relationshipsNode;
    var relationshipProperties = isRelationshipProperties(newRelationshipRef) && newRelationshipRef;

    if (!relationshipsNode) {
      relationshipsNode = archimateFactory.create(ARCHIMATE_RELATIONSHIPS, { relationships: [] });
      relationshipsNode.$parent = canvas._rootElement.modelRef;
      canvas._rootElement.modelRef.relationshipsNode = relationshipsNode;
    }

    if (relationshipProperties) {
      newRelationshipRef = oldRelationshipRef;
    }

    if (!newRelationshipRef) {
      newRelationshipRef = archimateFactory.create(ARCHIMATE_RELATIONSHIP, { type: connection.type });
      newRelationshipRef.name = connection.name;
      newRelationshipRef.$parent = relationshipsNode;
      relationshipsNode.relationships.push(newRelationshipRef);


      if (isRelationshipBaseType(connection.type, RELATIONSHIP_INFLUENCE, profile)) {
        setOptionalProperty(newRelationshipRef, 'modifier', getRelationshipOptionValue(connection, 'modifier'));
      }

      if (isRelationshipBaseType(connection.type, RELATIONSHIP_ACCESS, profile)) {
        setOptionalProperty(newRelationshipRef, 'accessType', getRelationshipOptionValue(connection, 'accessType'));
      }

      if (isRelationshipBaseType(connection.type, RELATIONSHIP_ASSOCIATION, profile)) {
        setOptionalProperty(newRelationshipRef, 'isDirected', getRelationshipOptionValue(connection, 'isDirected'));
      }

      setRelationshipEnds(newRelationshipRef, source, target);
      copyMultiplicityFromConnection(newRelationshipRef, connection);
    }

    if (relationshipProperties) {
      applyRelationshipProperties(newRelationshipRef, relationshipProperties, connection, profile);
      setRelationshipEnds(newRelationshipRef, source, target);
    }

    connection.name = newRelationshipRef.name;

    var type = newRelationshipRef.type;
    connection.type = type;
    delete connection.typeOption;
    delete connection.modifier;
    delete connection.accessType;
    delete connection.isDirected;

    if (isRelationshipBaseType(type, RELATIONSHIP_INFLUENCE, profile)) {
      setConnectionRelationshipOption(connection, 'modifier', newRelationshipRef.modifier);
    }

    if (isRelationshipBaseType(type, RELATIONSHIP_ACCESS, profile)) {
      setConnectionRelationshipOption(connection, 'accessType', newRelationshipRef.accessType);
    }

    if (isRelationshipBaseType(type, RELATIONSHIP_ASSOCIATION, profile)) {
      setConnectionRelationshipOption(connection, 'isDirected', newRelationshipRef.isDirected);
    }

    if (canApplyRelationshipMultiplicity(connection)) {
      setOptionalProperty(connection, 'sourceMultiplicity', normalizeRelationshipMultiplicity(newRelationshipRef.sourceMultiplicity));
      setOptionalProperty(connection, 'targetMultiplicity', normalizeRelationshipMultiplicity(newRelationshipRef.targetMultiplicity));
    } else {
      delete connection.sourceMultiplicity;
      delete connection.targetMultiplicity;
    }
    connection.businessObject.relationshipRef = newRelationshipRef;

    context.oldRelationshipRef = oldRelationshipRef;

    elements.push(connection);

    eventBus.fire('elements.changed', { elements });
    return elements;

  }

  /*
  var type = properties.type,
    reverse = properties.reverse,
    existingRelationship = properties.existingRelationship;

  if (existingRelationship) {
    connection.businessObject.relationshipRef = existingRelationship;
    connection.name = existingRelationship.name;
  }

  if (type) {
    connection.type = properties.type;

    if (type === RELATIONSHIP_INFLUENCE) {
      connection.modifier = properties.modifier;
      //relationshipRef.modifier = properties.modifier;
    }

    if (type === RELATIONSHIP_ACCESS) {
      connection.accessType = properties.accessType;
      //relationshipRef.accessType = properties.accessType;
    }

    if (type === RELATIONSHIP_ASSOCIATION) {
      connection.isDirected = properties.isDirected;
      //relationshipRef.isDirected = properties.isDirected;
    }

    if (reverse) {
      var newSource = connection.target;
      connection.target = connection.source;
      connection.source = newSource;

      var newWaypoints = connection.waypoints.reverse();

      this._connectionUpdater.updateConnectionWaypoints(context);
      //logger.log('updateWaypoints after reverse');
      //this._eventBus.fire('connection.updateWaypoints', { connection: connection });
      //this._modeling.updateWaypoints(connection, newWaypoints);

    }


  }

  this._connectionUpdater.updateConnectionObjects(context);

  return changed;
   */
};

function isRelationshipProperties(value) {
  return value && typeof value === 'object' &&
    !value.id &&
    !value.source &&
    !value.target &&
    (
      hasProperty(value, 'type') ||
      hasProperty(value, 'modifier') ||
      hasProperty(value, 'accessType') ||
      hasProperty(value, 'isDirected') ||
      hasProperty(value, 'sourceMultiplicity') ||
      hasProperty(value, 'targetMultiplicity')
    );
}

function applyRelationshipProperties(relationship, properties, connection, profile) {
  relationship.type = properties.type || connection.type || relationship.type;
  relationship.name = properties.name || connection.name || relationship.name;

  delete relationship.modifier;
  delete relationship.accessType;
  delete relationship.isDirected;

  if (isRelationshipBaseType(relationship.type, RELATIONSHIP_INFLUENCE, profile)) {
    setOptionalProperty(relationship, 'modifier', getRelationshipPropertyValue(properties, connection, 'modifier'));
  }

  if (isRelationshipBaseType(relationship.type, RELATIONSHIP_ACCESS, profile)) {
    setOptionalProperty(relationship, 'accessType', getRelationshipPropertyValue(properties, connection, 'accessType'));
  }

  if (isRelationshipBaseType(relationship.type, RELATIONSHIP_ASSOCIATION, profile)) {
    setOptionalProperty(relationship, 'isDirected', getRelationshipPropertyValue(properties, connection, 'isDirected'));
  }

  if (!canApplyRelationshipMultiplicity(connection)) {
    delete relationship.sourceMultiplicity;
    delete relationship.targetMultiplicity;
    return;
  }

  var sourceMultiplicity = hasProperty(properties, 'sourceMultiplicity') ?
    normalizeRelationshipMultiplicity(properties.sourceMultiplicity) :
    normalizeRelationshipMultiplicity(connection.sourceMultiplicity || relationship.sourceMultiplicity);

  var targetMultiplicity = hasProperty(properties, 'targetMultiplicity') ?
    normalizeRelationshipMultiplicity(properties.targetMultiplicity) :
    normalizeRelationshipMultiplicity(connection.targetMultiplicity || relationship.targetMultiplicity);

  setOptionalProperty(relationship, 'sourceMultiplicity', sourceMultiplicity);
  setOptionalProperty(relationship, 'targetMultiplicity', targetMultiplicity);
}

function copyMultiplicityFromConnection(relationship, connection) {
  if (!canApplyRelationshipMultiplicity(connection)) {
    delete relationship.sourceMultiplicity;
    delete relationship.targetMultiplicity;
    return;
  }

  setOptionalProperty(relationship, 'sourceMultiplicity', normalizeRelationshipMultiplicity(connection.sourceMultiplicity));

  setOptionalProperty(relationship, 'targetMultiplicity', normalizeRelationshipMultiplicity(connection.targetMultiplicity));
}

function getRelationshipPropertyValue(properties, connection, propertyName) {
  if (hasProperty(properties, propertyName)) {
    return properties[propertyName];
  }

  return getRelationshipOptionValue(connection, propertyName);
}

function getRelationshipOptionValue(element, propertyName) {
  var value = element[propertyName];

  if (value !== undefined && value !== null) {
    return value;
  }

  return element.typeOption;
}

function setConnectionRelationshipOption(connection, propertyName, value) {
  setOptionalProperty(connection, 'typeOption', value);
  setOptionalProperty(connection, propertyName, value);
}

function setRelationshipEnds(relationship, source, target) {
  if (is(source.businessObject, ARCHIMATE_NODE)) {
    relationship.source = source.businessObject.elementRef;
  }
  if (is(source.businessObject, ARCHIMATE_CONNECTION)) {
    relationship.source = source.businessObject.relationshipRef;
  }
  if (is(target.businessObject, ARCHIMATE_NODE)) {
    relationship.target = target.businessObject.elementRef;
  }
  if (is(target.businessObject, ARCHIMATE_CONNECTION)) {
    relationship.target = target.businessObject.relationshipRef;
  }
}

function setOptionalProperty(target, property, value) {
  if (value === undefined || value === null || value === '') {
    delete target[property];
  } else {
    target[property] = value;
  }
}

function hasProperty(value, property) {
  return Object.prototype.hasOwnProperty.call(value, property);
}

function isRelationshipBaseType(relationshipType, baseRelationshipType, profile) {
  var resolvedType = profile ?
    getBaseRelationshipTypeForProfile(relationshipType, profile) :
    relationshipType;

  return resolvedType === baseRelationshipType;
}

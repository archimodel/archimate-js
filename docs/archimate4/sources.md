...
- `scripts/write_archimate4_status_completion_scan.mjs` regenerates the implementation-status
  completion scan from `getArchimate4ImplementationStatus()` and can bind the emitted status to a
  caller-supplied runlog path for reproducible evidence refreshes.
...
- The validator checks `Elements` / `Relationships` container, list, and entry structure, element
  catalog membership, ArchiMate 3 concepts retired or merged in ArchiMate 4, relationship type
  membership, relationship endpoint membership, active relationship
...
- Refreshed again on 2026-07-09T20:12:26+09:00 in
  ...
  `archimate_*` candidate URLs returned 404, while the 3.1 Model XSD baseline returned 200.
  `archimate4-profile.json` recorded this check in `sourceCoverage.meff4Xsd`.
- Refreshed again on 2026-07-09T23:50:00+09:00 in

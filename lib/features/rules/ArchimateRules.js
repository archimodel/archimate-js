...
import {
  is, isAny, getElementRef, getViewElement, VIEW, NOTE } from '../../util/ModelUtil';
...
function canConnect(source, target, connection) {
  ...
  if (source.businessObject.type === 'Element') { //is(source.businessObject, 'archimate:Node')) {
    if (target.businessObject.type === 'Element') { //is(target.businessObject, 'archimate:Node')) {
    ...
    if (is(target.businessObject, 'archimate:Connection')) { //target.businessObject.type === 'Relationship') {
  ...
  if (source.type === NOTE) {
    if (is(target.businessObject, 'archimate:Node')) { // if (is(target.businessObject, 'archimate:Node') || target.type === NOTE)) {
      return { type: 'Line' };
    }
  }

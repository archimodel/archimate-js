...
Use `getArchimate4ImplementationStatus()` to inspect the current ArchiMate 4 implementation boundary. It reports the implemented 42-element catalog, active relationship profile status, experimental MEFF 4 exchange status, local icon coverage including generic object alias count, C260 conformance requirement status, C260 section coverage status, W262 change-rationale coverage, Appendix C example viewpoint catalog status, source coverage status, conformance readiness, remaining gap identity, and the external blockers that must be resolved before official conformance can be claimed.

Use `getArchimate4ConformanceReport()` when a host needs a flattened official-conformance report. It preserves the Appendix B profile-data, MEFF 4.0 XSD, and Appendix A artwork-rights blockers, reports W262 as a reviewed local companion source, and lists the required action for each blocker without allowing local implementation coverage to be mistaken for an official conformance claim.
...
Run `node scripts/audit_archimate4_completion.mjs --out project_memory/runlogs/<file>.json` to map the current status API to the plan's M0-M5 milestones and to verify that referenced runlogs resolve. The audit is expected to pass while still reporting the official Appendix B profile-data, MEFF 4.0 XSD, and Appendix A artwork-rights gaps; W262 is no longer a companion-source gap.

Run `node scripts/write_archimate4_status_completion_scan.mjs --out project_memory/runlogs/<file>.json` to regenerate the implementation-status completion scan from `getArchimate4ImplementationStatus()` before refreshing status evidence.

Run `npm run archimate4:relationship-template -- --out output/archimate4-appendix-b-relationship-profile-template.tsv` to generate a blank Appendix B transcription matrix containing every ArchiMate 4 source-target concept pair. Fill each cell with the supported relationship codes before importing it through the Editor demo or `setArchimate4RelationshipProfile()`. A complete but entirely empty template is rejected so it cannot silently replace the compatibility fallback with a profile that allows no relationships.
...
The Appendix B source-coverage entry records personal-use authorization separately from profile-data availability. Redistribution is not required for this deployment, but the licensed C260 source's AI/automated-processing restriction remains in force and no complete machine-readable profile is present. The loader, blank template generator, activation guard, validation, status, and coverage report APIs are implemented; the matrix must be supplied as human-prepared or separately authorized JSON, CSV, or TSV data.

The W262 source-coverage entry records the reviewed local 22-page PDF, its SHA-256 and topic ids, the latest scripted Downloads/attachments search, and publication-page status. `w262ChangeCoverage` maps all 11 reviewed change-rationale topics to page evidence and implementation status keys. It is a companion source and no longer appears in `missingCompanionSources` or `remainingGaps`.

The local ArchiMate 4 Non-Commercial License files are tracked as license-boundary context only. Appendix B personal-use authorization is recorded separately; the Appendix A exact-artwork-rights blocker remains unresolved.
...
* Profile attribute value helpers store values as reusable model `PropertyDefinition` entries named `archimate-js:profileAttribute:<concept>:<name>` plus per-concept `Properties`, preserving typed values without adding non-standard element fields; model validation checks `PropertyDefinitions` / `Properties` container structure, reusable `PropertyDefinition` entry structure, present `Property.value` string values, and resolves `Property.propertyDefinitionRef` id references before profile attribute value checks.
* Viewpoint metadata can be retained on views through `viewpoint` or `viewpointRef`, and custom viewpoint definitions can be supplied in `archimateLanguageProfile.viewpoints`; `Viewpoints` container structure, viewpoint purpose/content token-field structure and values, and allowed element/relationship types are validated against the active profile and applied to referenced View contents through either model-defined `viewpointRef` or profile-defined `viewpoint` names.
...
* Model organization trees can be retained through `organizationsNode`, with nested `Organization` entries and optional references back to model concepts; model validation checks the organization container/list shape and those `identifierRef` values against active model concepts.
...
* `validateArchimate4Model(model, options)` and `validateArchimateModel(model, options)` expose model-level diagnostics for unsupported 3.x/4.0 concepts, retired ArchiMate 3 concepts, `Elements` / `Relationships` container structure, relationship endpoint references and type support, relationship option field types, active relationship profile checks, multiplicity notation, junction relationship consistency, present `IdObject.id` and `xsi:type` string values, `Views` / `Diagrams` / `View` / `ViewElement` tree structure, view `viewpoint` string attributes and `viewpointRef` references, view node `elementRef` references, view element `label` string values and geometry, view connection `relationshipRef`, endpoint references, endpoint-to-relationship alignment, waypoint geometry, and view `style`/`font`/`color` values including invalid `font.style` `plain` combinations, `Viewpoints` container structure, viewpoint purpose/content token-field structure and values, allowed-type string entries, stakeholder/concern, and `modelingNotes` structure, model-defined and profile-defined viewpoint allowed-type application to View contents, `BaseObject` `name` / `documentation` string fields, organization tree structure and `identifierRef` references, `PropertyDefinitions` / `Properties` container structure, reusable `PropertyDefinition` structure, `Property.propertyDefinitionRef` references, present `Property.value` string structure, and profile attribute property values.
...
* The current implementation-status completion API scan is recorded in `project_memory/runlogs/20260710-0616-status-completion-api-scan.json`; it records 50 top-level status keys, 41 `complete` summaries, no incomplete summaries, and the section coverage status-key guard arrays including `exampleViewpointCatalog` and `w262ChangeCoverage`.

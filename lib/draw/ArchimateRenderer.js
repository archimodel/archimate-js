...
export default function ArchimateRenderer(
  ...
  function drawShapePath(parentGfx, d, attrs) {
    attrs = computeStyle(attrs, {
      stroke: DEFAULT_STROKE_COLOR,
      strokeWidth: DEFAULT_STROKE_WIDTH,
      fill: DEFAULT_FILL_COLOR
  ...
  function renderArchimateConnection(parentGfx, connection, attrs) {
    ...
    if (sourceMultiplicity && waypoints.length) {
      renderLabel(parentGfx, sourceMultiplicity, {
        box: {
          x: waypoints[0].x + 6,
          y: waypoints[0].y + 6,
          width: 40,
    ...
    if (targetMultiplicity && waypoints.length) {
      var last = waypoints[waypoints.length - 1];
      renderLabel(parentGfx, targetMultiplicity, {
        box: {
          x: last.x - 46,
          y: last.y + 6,
          width: 40,
    ...
    if (modifier && waypoints.length) {
      var position = getConnectionMidpoint(waypoints);
      renderLabel(parentGfx, modifier, {
        box: {
          x: position.x - 20,
          y: position.y - 20,
          width: 40,
  ...
  function getInfluenceModifier(connection) {
    var relationshipType = profile ?
      getBaseRelationshipTypeForProfile(connection.type, profile) :
      connection.type;

    if (relationshipType !== RELATIONSHIP_INFLUENCE) {
      return '';
    }

    var modifier = getRelationshipOptionValue(connection, 'modifier');
  ...
  function getAccessType(connection) {
    return getRelationshipOptionValue(connection, 'accessType');
  }

  function isAssociationDirected(connection) {
    return isRelationshipFlagEnabled(getRelationshipOptionValue(connection, 'isDirected'));
  }
  ...
  function getRelationshipOptionValue(element, propertyName) {
    ...
    if (element.typeOption !== undefined && element.typeOption !== null) {
  ...
  function isRelationshipFlagEnabled(value) {
  ...
  function getJunctionDisplayLabel(shape, elementType) {
    ...
    if (!label || label === elementType ||
        label === 'AND' || label === 'OR' ||
        label === 'And Junction' || label === 'Or Junction') {
      return '';
    }
  ...
  function isArchimate4Profile(profile) {
    return profile && String(profile.version).indexOf('4') === 0;
  }

  function isArchimate4MotivationShape(shape) {
    return isArchimate4Profile(profile) && shape.aspect === 'Motivation';
  }

  function getShapeRectAttrs(shape, elementType) {
    var attrs = {
      fill: shape.style.fillColor,
      stroke: shape.style.lineColor
    };

    if (isArchimate4Profile(profile) && elementType === OTHER_GROUPING) {
      attrs.fill = 'none';
  ...
  function getShapePictogramFill(shape, elementType) {
    if (isArchimate4Profile(profile) && elementType === OTHER_GROUPING) {
      return DEFAULT_STROKE_COLOR;
    }

    return shape.style.fillColor;
  }

  function getJunctionCircleAttrs(shape, elementType) {
    if (isArchimate4Profile(profile) && elementType === RELATIONSHIP_JUNCTION_AND) {
      return {
    ...
    if (isArchimate4Profile(profile) && elementType === RELATIONSHIP_JUNCTION_OR) {
      return {
        fill: 'none',
        stroke: DEFAULT_STROKE_COLOR
      };
  ...
  function drawArchimateBody(parentGfx, shape, borderRadius, attrs) {
    if (isArchimate4MotivationShape(shape)) {
      return drawShapePath(parentGfx, getChamferedRectPath({
        x: 0,
        y: 0,
        width: shape.width,
        height: shape.height
      }, MOTIVATION_CHAMFER_SIZE), attrs);
    }

    return drawRect(parentGfx, shape.width, shape.height, borderRadius, attrs);
  }
  ...
  function renderArchimateShape(parentGfx, shape) {
    ...
    var attrs = getShapeRectAttrs(shape, elementType);

    if (elementType === RELATIONSHIP_JUNCTION_AND || elementType === RELATIONSHIP_JUNCTION_OR) {
      var circle = drawCircle(parentGfx, shape.width, shape.height, 1, getJunctionCircleAttrs(shape, elementType));

      if (!isArchimate4Profile(profile)) {
        renderLabel(parentGfx, elementType === RELATIONSHIP_JUNCTION_AND ? 'AND' : 'OR', {
          box: {
            x: 0,
            y: Math.round(shape.height / 2) - 9,
            width: shape.width,
            height: 18
          },
          align: 'center-middle',
      ...
      var junctionLabel = getJunctionDisplayLabel(shape, elementType);

      if (junctionLabel) {
        renderLabel(parentGfx, junctionLabel, {
          box: {
            x: -20,
            y: shape.height + 2,
    ...
    var figure = drawArchimateBody(parentGfx, shape, borderRadius, attrs);
    ...
    var pictoRef = getPictoRef(elementType, profile);
    ...
    drawPath(parentGfx, pathData, {
      strokeWidth: 0.8,
      fill: getShapePictogramFill(shape, elementType), // getFillColor(shape, defaultColorLayer),
      stroke: DEFAULT_STROKE_COLOR
    });
  ...
  this.handlers = {
    ...
    'archimate:Group': function(parentGfx, element) {
      ...
      var align = element.style.textAlignment+'-'+element.style.textPosition, // || 'center-middle',
          fontWeight = element.style.fontStyle;
    ...
    'Access': function(parentGfx, connection) {
      ...
      var accessType = getAccessType(connection);

      if (accessType === RELATIONSHIP_ACCESS_WRITE ||
          accessType === RELATIONSHIP_ACCESS_READWRITE) {
        attrs.markerEnd = marker('opened-end', color);
      }

      if (accessType === RELATIONSHIP_ACCESS_READ ||
          accessType === RELATIONSHIP_ACCESS_READWRITE) {
    ...
    'Association': function(parentGfx, connection) {
      ...
      if (isAssociationDirected(connection)) {
...
ArchimateRenderer.prototype.drawConnection = function(parentGfx, element) {
  ...
  if (!h && this._languageProfile) {
    var profile = this._languageProfile.get();
    var baseType = getBaseRelationshipTypeForProfile(type, profile);

    h = this.handlers[baseType];
  }

import { BUSINESS_ACTOR, BUSINESS_FUNCTION, BUSINESS_INTERFACE, BUSINESS_PROCESS, APPLICATION_PROCESS, RELATIONSHIP_ACCESS, RELATIONSHIP_AGGREGATION, RELATIONSHIP_ASSIGNMENT, RELATIONSHIP_ASSOCIATION, RELATIONSHIP_COMPOSITION, RELATIONSHIP_FLOW, RELATIONSHIP_INFLUENCE, RELATIONSHIP_REALIZATION, RELATIONSHIP_SERVING, RELATIONSHIP_SPECIALIZATION, RELATIONSHIP_TRIGGERING } from "../../util/ModelUtil";
...
const RELATIONSHIPS_ALLOWED_BY_ELEMENT_MAP = new Map([
    [BUSINESS_ACTOR, 'scgirvantfo'], 
    [BUSINESS_FUNCTION, 'scgrvantfo'],
    [BUSINESS_INTERFACE, 'scgirvantfo'],
    [BUSINESS_PROCESS, 'scgrvantfo'],
    [APPLICATION_PROCESS, 'sctfo']
]);
...
export function getRelationshipsAllowedByElementType(elementType) {
    var relationshipsAllowed = [];

    if (elementType && RELATIONSHIPS_ALLOWED_BY_ELEMENT_MAP.has(elementType)) {
        var relationshipsAllowedString = RELATIONSHIPS_ALLOWED_BY_ELEMENT_MAP.get(elementType);
        var menuDef = null;

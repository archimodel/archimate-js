...
export default function ArchimateRenderer(
  ...
  function renderArchimateShape(parentGfx, shape) {
    ...
    drawPath(parentGfx, pathData, {
      strokeWidth: 0.8,
      fill: shape.fillColor, // getFillColor(shape, defaultColorLayer),
      stroke: DEFAULT_STROKE_COLOR
    });
  ...
  this.handlers = {
    'archimate:Node': function(parentGfx, shape) {
      ...
      if (shape.type === NOTE) {
        ...
        var align = shape.textAlign || 'left-top',
        fontWeight = shape.fontStyle;
...
ArchimateRenderer.prototype.canRender = function(element) {
  ...
  return isAny(element.businessObject, ['archimate:Node', 'archimate:Connection']);
};

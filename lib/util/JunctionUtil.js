...
export function getJunctionRelationshipTypeCandidates(source, target, exceptConnection, relationshipAllowed, profile) {
  var types = getExistingJunctionRelationshipTypes(source, target, exceptConnection, profile);
...
function getExistingJunctionRelationshipTypes(source, target, exceptConnection, profile) {
  var junctions = [];
  ...
  for (const junction of junctions) {
    var types = getRelationshipTypesConnectedToJunction(junction, exceptConnection, profile);
...
function getRelationshipTypesConnectedToJunction(junction, exceptConnection, profile) {
  ...
  for (const connection of connections) {
    ...
    var type = getRelationshipType(connection, profile);
...
function getRelationshipType(connection, profile) {
  ...
  var baseType = profile ?
    getBaseRelationshipTypeForProfile(type, profile) :
    type;

  return ARCHIMATE_RELATIONSHIP_TYPES.indexOf(baseType) !== -1 ? baseType : null;
}

function getBaseRelationshipTypeForProfile(relationshipType, profile) {
...
function getProfileRelationship(relationshipType, profile) {
  ...
  for (const relationship of relationships) {
    var type = typeof relationship === 'string' ? relationship : relationship && relationship.type;

    if (type === relationshipType) {
      return typeof relationship === 'string' ? { type: relationship } : relationship;
    }
  }

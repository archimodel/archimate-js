...
* Viewpoint metadata can be retained on views through `viewpoint` or `viewpointRef`, and custom viewpoint definitions can be supplied in `archimateLanguageProfile.viewpoints`; `Viewpoints` container structure, viewpoint purpose/content values, and allowed element/relationship types are validated against the active profile and applied to referenced View contents through either model-defined `viewpointRef` or profile-defined `viewpoint` names.

 ...
 */
export default function ArchimateRules(eventBus, languageProfile) {
...
ArchimateRules.$inject = [ 'eventBus', 'languageProfile' ];
...
ArchimateRules.prototype.init = function() {
  ...
  this.addRule('connection.create', function(context) {
    ...
    return canConnect(source, target, null, profile);
  });
  ...
  this.addRule('connection.reconnect', function(context) {
    ...
    return canConnect(source, target, connection, profile);
  });
...
function canConnect(source, target, connection, profile) {
  ...
  if (source.businessObject.type === NODE_ELEMENT) { // is(source.businessObject, ARCHIMATE_NODE)) {
    if (target.businessObject.type === NODE_ELEMENT) { // is(target.businessObject, ARCHIMATE_NODE)) {
      ...
      if (aggregationType) {
        return canKeepRelationshipType(connection, aggregationType);
      }

      // connection.reconnect call
      if (connection) {
        if (isRelationshipConnectedToJunction({ source: source, target: target })) {
          return isJunctionRelationshipTypeAllowed(source, target, connection.type, connection, isRelationshipAllowed, profile) ?
            { type: connection.type } :
            false;
        }

        if (isRelationshipAllowed(source.type, target.type, connection.type, profile)) {
    ...
    if (is(target.businessObject, ARCHIMATE_CONNECTION)) { // target.businessObject.type === CONNECTION_RELATIONSHIP) {
      var relationshipAggregationType = getRelationshipConceptAggregationType(source, target, profile);

      if (relationshipAggregationType) {
        return canKeepRelationshipType(connection, relationshipAggregationType);
      }
...
function canKeepRelationshipType(connection, relationshipType) {
  if (connection && connection.type !== relationshipType) {
    return false;
  }

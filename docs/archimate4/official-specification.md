...
diagnostics cover concept container structure, catalog membership, retired concepts, relationship endpoint references and endpoint type support, relationship option
...
- `validateArchimate4Model()` reports unsupported relationship types, unsupported endpoint concept
  ...
  invalid endpoint chains through a junction, invalid concept container structure, invalid view tree structure, unknown or malformed view viewpoint references, invalid
...
- On 2026-07-10T06:01:25+09:00, the official directory evidence was refreshed again in
...
- `getArchimate4ImplementationStatus().modelValidation` must expose expected/actual/missing/extra
  validation check ids so model-level diagnostics cannot silently drop concept container structure,
  element catalog, retired concept, relationship profile, multiplicity, junction consistency,
  view tree structure, view viewpoint-attribute and

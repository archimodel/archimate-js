...
* `CORE`: added malformed and unknown relationship source/target reference diagnostics for ArchiMate model validation.
* `CORE`: added invalid organization tree container, list, and entry diagnostics for ArchiMate model validation.
* `CORE`: extended model-level ArchiMate 4 diagnostics to validate present IdObject ids, `xsi:type` fields, `Elements` / `Relationships`, `Viewpoints`, and `PropertyDefinitions` / `Properties` container structure, relationship option field types, view tree structure, view viewpoint string attributes and references, view node concept references, view element labels and geometry, view connection concept references, endpoints, endpoint-to-relationship alignment, waypoint geometry, view style/font/color values including invalid `font.style` `plain` combinations, model-defined and profile-defined viewpoint allowed-type application to View contents, viewpoint purpose/content token-field structure, viewpoint stakeholder/concern and modeling-note structure, BaseObject name/documentation string fields, organization tree structure and identifier references, property definition structure and references, present property value string structure, viewpoint definitions including allowed-type string entries, and profile attribute property values.
...
* `CORE`: added a reproducible ArchiMate 4 implementation-status completion scan writer and refreshed the current scan evidence.
...
* `FEAT`: exposed the ArchiMate 4 conformance report in the bundled Viewer and Editor demos while keeping ArchiMate 3.x marked as not applicable.
* `FEAT`: exposed the informative Appendix C example viewpoint catalog in the bundled Viewer and Editor demos for ArchiMate 4 mode.
* `FEAT`: exposed model-defined Viewpoint validation status in the bundled Viewer and Editor demos for ArchiMate 4 mode.
* `FEAT`: added an Editor demo Appendix B relationship profile import panel for host-supplied JSON, CSV, or TSV profile data, with complete cell coverage required in ArchiMate 4 mode.

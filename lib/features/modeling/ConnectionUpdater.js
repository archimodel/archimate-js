...
ConnectionUpdater.prototype.updateConnectionObjects = function(context) {
  ...
  if (connection.businessObject.type === CONNECTION_RELATIONSHIP) {
    ...
    if (connection.type === RELATIONSHIP_INFLUENCE) {
      relationship.modifier = connection.modifier || connection.typeOption;
    }

    if (connection.type === RELATIONSHIP_ACCESS) {
      relationship.accessType = connection.accessType !== undefined ? connection.accessType : connection.typeOption;
    }

    if (connection.type === RELATIONSHIP_ASSOCIATION) {
      relationship.isDirected = connection.isDirected !== undefined ? connection.isDirected : connection.typeOption;
    }

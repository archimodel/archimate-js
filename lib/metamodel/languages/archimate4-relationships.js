...
export function setArchimate4RelationshipProfile(profile, options) {
  ...
  var normalizedRelationships = normalizeRelationshipProfile(
    parsedProfile,
    VALID_ARCHIMATE4_CONCEPT_TYPES,
    loadOptions
  );
  var normalizedStats = getRelationshipProfileStats(normalizedRelationships);

  if (loadOptions.requireRelationships && normalizedStats.relationshipCount === 0) {
    throw new Error('ArchiMate 4 relationship profile does not contain any allowed relationships');
  }
  ...
  RELATIONSHIP_PROFILE_OPTIONS = {
    ...
    requireRelationships: !!loadOptions.requireRelationships
  };
...
export function getArchimate4RelationshipProfileStatus() {
  ...
  return {
    ...
    requireRelationships: RELATIONSHIP_PROFILE_OPTIONS.requireRelationships,

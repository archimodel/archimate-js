...
import { DEFAULT_NOTE_COLOR, BLACK_SHADOW, getDomainColor, DEFAULT_CONNECTION_COLOR } from '../../util/ColorUtil';
 ...
 *
 */
export default function ElementFactory(archimateFactory, moddle, translate, languageProfile) {
...
ElementFactory.prototype.createConnection = function(attrs) {
  ...
  assertRelationshipAvailable(type, profile, translate);
  ...
  if (archimateConnection) {
    ...
    setAttrsRelationshipRef(connectionAttrs, archimateConnection.relationshipRef, profile);
    ...
    if (attrs.relationshipRef) {
      ...
      setAttrsRelationshipRef(connectionAttrs, attrs.relationshipRef, profile);
    }
...
ElementFactory.prototype.createShape = function(attrs) {
  ...
  assertConceptAvailable(attrs.type, profile, translate);
  ...
  if (archimateNode) {
    ...
    if (attrs.type !== NOTE) {
      layer = getLayerType(attrs.type, profile);
      domain = getDomainType(attrs.type, profile);

      attrs = assign({
        name: archimateNode.elementRef && archimateNode.elementRef.name,
        layer: layer,
        domain: domain,
        aspect: getAspectType(attrs.type, profile),
      }, attrs);
    }
    ...
    if (attrs.type === NOTE) {
      ...
      assign(style, {
        fillColor: getDomainColor(domain || layer, profile),
        lineColor: BLACK_SHADOW,
      ...
      attrs = assign({
        name: getTypeName(attrs.type, profile),
        layer: layer,
        domain: domain,
        aspect: getAspectType(attrs.type, profile),
      }, attrs);
...
function setAttrsRelationshipRef(attrs, relationshipRef, profile) {
  var canApplyMultiplicity = canApplyRelationshipMultiplicity(attrs);
  var baseRelationshipType = profile ?
    getBaseRelationshipTypeForProfile(attrs.type, profile) :
    attrs.type;
  var sourceMultiplicity,
      targetMultiplicity;

  if (canApplyMultiplicity && relationshipRef) {
    sourceMultiplicity = normalizeRelationshipMultiplicity(relationshipRef.sourceMultiplicity);
    targetMultiplicity = normalizeRelationshipMultiplicity(relationshipRef.targetMultiplicity);
  }
  ...
  if (baseRelationshipType === 'Access') {
    assign(attrs, {
      typeOption: relationshipRef && relationshipRef.accessType,
      accessType: relationshipRef && relationshipRef.accessType
    });
  }

  if (baseRelationshipType === 'Association') {
    assign(attrs, {
      typeOption: relationshipRef && relationshipRef.isDirected,
      isDirected: relationshipRef && relationshipRef.isDirected
    });
  }

  if (baseRelationshipType === 'Influence') {
    assign(attrs, {
      typeOption: relationshipRef && relationshipRef.modifier,
      modifier: relationshipRef && relationshipRef.modifier
    });
  }
}

function assertConceptAvailable(type, profile, translate) {
  if (!profile || type === NOTE || hasProfileConcept(type, profile)) {
    return;
  }

  throw new Error(translate('ArchiMate concept {type} is not available in ArchiMate {version}', {
    type: type,
    version: profile && profile.version || 'default'
  }));
}

function assertRelationshipAvailable(type, profile, translate) {
  if (type === CONNECTION_LINE || type === CONNECTION_RELATIONSHIP || hasProfileRelationship(type, profile)) {
    return;
  }

  throw new Error(translate('ArchiMate relationship {type} is not available in ArchiMate {version}', {
    type: type,
    version: profile && profile.version || 'default'
  }));
}

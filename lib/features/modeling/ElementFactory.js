...
ElementFactory.prototype.create = function(elementType, attrs) {
  ...
  if (elementType === 'connection') {
    ...
    if (!connection) {
      ...
      var relationship = this._archimateFactory.create('archimate:Relationship', { type: 'archimate:Relationship' });

      connection = this._archimateFactory.create('archimate:Connection', { relationshipRef: relationship });
    ...
    attrs = assign({
      ...
      id: connection.id,
      type: connection.relationshipRef.type
    }, attrs);
  ...
  if (elementType === 'shape') {
    ...
    if (!viewElement) {
      ...
      // check if given type is a valid BaseElement type (eg. 'BusinessActor', 'ApplicationFunction',...)
      if (isBaseElementType(attrs.type)) {
        ...
        logger.log('new BaseElement => createViewElement in ElementFactory');
        ...
        logger.log('return of createViewElement in ElementFactory');
        logger.log(viewElement);

      } else if (attrs.type === 'archimate:Note') {
        ...
        logger.error('Invalid shape type : ' + attrs.type);
        throw new Error(translate('shape type invalid'));
      }
    }
...
ElementFactory.prototype._getDefaultSize = function(elementType) {
  if (isBaseElementType(elementType)) {

...
var RELATIONSHIP_PROFILE_COVERAGE = getFallbackCoverageStats(RELATIONSHIPS);
...
export function setArchimate4RelationshipProfile(profile, options) {
  ...
  RELATIONSHIP_PROFILE_COVERAGE = getRelationshipProfileCoverageStats(profile, VALID_ARCHIMATE4_CONCEPT_TYPES);
...
export function resetArchimate4RelationshipProfileForTests() {
  ...
  RELATIONSHIP_PROFILE_COVERAGE = getFallbackCoverageStats(RELATIONSHIPS);
...
function getFallbackCoverageStats(maps) {
  var stats = getRelationshipProfileStats(maps);
  var conceptCount = VALID_ARCHIMATE4_CONCEPT_TYPES.size;

  return {
    sourceCount: stats.sourceCount,
    targetCellCount: stats.relationshipCount,
    expectedSourceCount: conceptCount,
    expectedTargetCellCount: conceptCount * conceptCount,
    completeSourceCoverage: stats.sourceCount === conceptCount,
    completeTargetCoverage: false
  };
}

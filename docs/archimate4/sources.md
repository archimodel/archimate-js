...
- `getArchimate4ImplementationStatus().modelValidation` reports `modelValidation.expectedCheckIds`,
  ...
  references, view node element references, view element `label` string values and geometry, view connection relationship references, endpoint
...
- The validator checks element catalog membership, ArchiMate 3 concepts retired or merged in
  ...
  and `modelingNotes` structure, ViewElement label string values, `BaseObject` name/documentation string fields, reusable

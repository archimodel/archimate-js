...
ConnectionUpdater.prototype.updateConnectionObjects = function(context) {
  ...
  if (connection.businessObject.type === CONNECTION_RELATIONSHIP) {
    ...
    if (canApplyRelationshipMultiplicity(connection) && connection.sourceMultiplicity) {
      relationship.sourceMultiplicity = connection.sourceMultiplicity;
    }

    if (canApplyRelationshipMultiplicity(connection) && connection.targetMultiplicity) {

...
* `validateArchimate4Model(model, options)` and `validateArchimateModel(model, options)` expose model-level diagnostics for unsupported 3.x/4.0 concepts, retired ArchiMate 3 concepts, relationship endpoint/type support, active relationship profile checks, multiplicity notation, junction relationship consistency, view viewpoint references, view node `elementRef` references, view connection `relationshipRef` references, viewpoint purpose/content and allowed-type definitions, model-defined and profile-defined viewpoint allowed-type application to View contents, viewpoint stakeholder/concern structure, organization `identifierRef` references, `Property.propertyDefinitionRef` references, and profile attribute property values.
...
* The current implementation-status completion API scan is recorded in `project_memory/runlogs/20260709-2303-status-completion-api-scan.json`; it records 49 top-level status keys, 40 `complete` summaries, no incomplete summaries, and the section coverage status-key guard arrays including `exampleViewpointCatalog`.

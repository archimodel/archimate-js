...
AttachmentBehavior.prototype.updateNodes = function(shape, newHost, oldHost) {
  ...
  if (oldHost) {
    removeNodeFromParent(oldHost.businessObject, node);
  }

  if (newHost) {
    addNodeToParent(newHost.businessObject, node);
  } else {
    delete node.$parent;
  }
...
AttachmentBehavior.prototype.updateConnections = function(shape, newHost, oldHost) {
  ...
  if (shape.type !== NOTE) {
    ...
    // check if there is nested relationship from shape to oldHost and create connection
    var nestedRelationships = getExistingRelationships(shape, oldHost, relationshipsNode);
    for (const relationship of nestedRelationships) {
      allRelationships.push({ relationshipRef: relationship, source: shape, target: oldHost });
    }

    // check if there is nested relationship from oldHost to shape
    var hostRelationships = getExistingRelationships(oldHost, shape, relationshipsNode);
    for (const relationship of hostRelationships) {
  ...
  // keep relationship in relationshipsNode
...
function removeNodeFromParent(parentBusinessObject, node) {
  if (is(parentBusinessObject, ARCHIMATE_NODE) && parentBusinessObject.nodes) {
    collectionRemove(parentBusinessObject.nodes, node);
  }

  if (is(parentBusinessObject, ARCHIMATE_VIEW) && parentBusinessObject.viewElements) {
    collectionRemove(parentBusinessObject.viewElements, node);
  }
}

function addNodeToParent(parentBusinessObject, node) {
  var nodes = getNodeCollection(parentBusinessObject);
  ...
  node.$parent = parentBusinessObject;
}

function getNodeCollection(parentBusinessObject) {
  if (is(parentBusinessObject, ARCHIMATE_NODE)) {
    if (!parentBusinessObject.nodes) {
      parentBusinessObject.nodes = [];
    }

    return parentBusinessObject.nodes;
  }

  if (is(parentBusinessObject, ARCHIMATE_VIEW)) {
    if (!parentBusinessObject.viewElements) {
      parentBusinessObject.viewElements = [];
    }

    return parentBusinessObject.viewElements;
  }
}

...
const BEHAVIOR_REALIZATION_ALTERNATIVES = [ 'Specialization', 'Aggregation' ];
const MERGED_BEHAVIOR_TYPES = new Set([ 'Process', 'Function', 'Service', 'Event' ]);
const MERGED_BEHAVIOR_ORIGINAL_DOMAINS = new Set([ 'Business', 'Application', 'Technology' ]);
...
export function migrateArchimate3ModelTo4(model, options = {}) {
  ...
  warnForCrossDomainBehaviorRealizations(model, warnings, options);
...
function warnForCrossDomainBehaviorRealizations(model, warnings, options) {
  if (options.warnBehaviorRealizationAlternatives === false ||
      options.warnServiceRealizationAlternatives === false) {
    return;
  }
  ...
  relationships.forEach(function(relationship) {
    ...
    if (relationship.type !== 'Realization' ||
        !MERGED_BEHAVIOR_TYPES.has(sourceType) ||
        !MERGED_BEHAVIOR_TYPES.has(targetType)) {
      return;
    }

    if (!isMergedBehaviorDomain(sourceDomain) || !isMergedBehaviorDomain(targetDomain)) {
      return;
    }
    ...
    warnings.push({
      ...
      alternativeReplacementTypes: BEHAVIOR_REALIZATION_ALTERNATIVES.slice(),
      sourceType,
      targetType,
      sourceOriginalDomain: sourceDomain,
      targetOriginalDomain: targetDomain,
      message: 'Realization between merged behavior elements from different ArchiMate 3 domains is model-dependent after ArchiMate 4 migration; verify whether Specialization or Aggregation is more precise.'
    });
  });
}
...
function isMergedBehaviorDomain(domain) {
  return MERGED_BEHAVIOR_ORIGINAL_DOMAINS.has(domain);
}

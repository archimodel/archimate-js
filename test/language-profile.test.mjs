...
function collectCompleteStatusSummaries(value, path = []) {
  if (!value || typeof value !== 'object') {
    return [];
  }
  ...
  for (const [ key, child ] of Object.entries(value)) {
    summaries.push(...collectCompleteStatusSummaries(child, path.concat(key)));
  }
...
test('archimate 4 implementation status completion scan stays parseable', async () => {
  ...
  const summaryIdentity = summaries.map((summary) => {
    return {
      path: summary.path,
      complete: summary.complete
    };
  });
  const runlogSummaryIdentity = runlog.completeSummaries.map((summary) => {
    return {
      path: summary.path,
      complete: summary.complete
    };
  ...
  assert.equal(completion.completeSummaryCount, summaries.length);
  assert.equal(completion.incompleteSummaryCount, 0);
  assert.deepEqual(completion.incompleteSummaryPaths, []);
  assert.equal(completion.complete, true);
  assert.equal(stderr, '');
  assert.deepEqual(runlog.topKeys, Object.keys(status));
  ...
  assert.deepEqual(runlog.sectionCoverageStatusKeys, status.sectionCoverage.statusKeyIds);
  assert.deepEqual(runlog.sectionCoverageMissingStatusKeyIds, status.sectionCoverage.missingStatusKeyIds);
  assert.deepEqual(runlog.sectionCoverageExtraStatusKeyIds, status.sectionCoverage.extraStatusKeyIds);
  assert.deepEqual(
...
test('archimate 4 implementation status exposes source coverage boundaries', async () => {
  ...
  assert.equal(sourceCoverage.w262.lastPublicationPageCheckedAt, '2026-07-10T03:35:00+09:00');
  assert.equal(
  ...
  assert.equal(sourceCoverage.w262.lastLocalSearchAt, '2026-07-10T03:35:00+09:00');
  assert.equal(
  ...
  assert.match(sources, /20260710-0173-external-source-current-recheck/);
  ...
  assert.match(officialSpec, /20260710-0173-external-source-current-recheck/);

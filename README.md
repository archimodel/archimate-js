...
* `validateArchimate4Model(model, options)` and `validateArchimateModel(model, options)` expose model-level diagnostics for unsupported 3.x/4.0 concepts, retired ArchiMate 3 concepts, relationship endpoint/type support, active relationship profile checks, multiplicity notation, junction relationship consistency, view viewpoint references, view node `elementRef` references, view connection `relationshipRef` references, viewpoint purpose/content, allowed-type, stakeholder/concern, and `modelingNotes` structure, model-defined and profile-defined viewpoint allowed-type application to View contents, organization `identifierRef` references, `Property.propertyDefinitionRef` references, and profile attribute property values.

...
export function validateArchimateModel(model, options = {}) {
  ...
  validateViewpointsNode(viewsNode, diagnostics);
...
function validateViewpointsNode(viewsNode, diagnostics) {
  if (!isObject(viewsNode)) {
    return;
  }

  var viewpointsNode = getModelViewpointsNode(viewsNode);

  if (viewpointsNode === undefined || viewpointsNode === null) {
    return;
  }

  if (Object.prototype.hasOwnProperty.call(viewsNode, 'viewpointsNode') && !isObject(viewpointsNode)) {
    addDiagnostic(diagnostics, 'error', 'invalid-viewpoints-node', 'Viewpoints node must be an object.', {
      valueType: typeof viewpointsNode
    });
    return;
  }

  validateViewpointListNode(viewpointsNode, diagnostics);
}

function validateViewpointListNode(viewpointsNode, diagnostics) {
  if (!Object.prototype.hasOwnProperty.call(viewpointsNode, 'viewpoints') ||
      viewpointsNode.viewpoints === undefined ||
      viewpointsNode.viewpoints === null) {
    return;
  }

  if (!Array.isArray(viewpointsNode.viewpoints)) {
    addDiagnostic(diagnostics, 'error', 'invalid-viewpoint-list', 'Viewpoint entries must be an array.', {
      field: 'viewpoints',
      valueType: typeof viewpointsNode.viewpoints
    });
    return;
  }

  viewpointsNode.viewpoints.forEach(function(viewpoint, viewpointIndex) {
    if (!isObject(viewpoint)) {
      addDiagnostic(diagnostics, 'error', 'invalid-viewpoint-entry', 'Viewpoint entries must be objects.', {
        viewpointIndex: viewpointIndex,
        valueType: typeof viewpoint
      });
    }
  });
}
...
function validateViewpointTokens(viewpoint, fieldName, allowedValues, unsupportedCode, invalidEntryCode, diagnostics) {
  var tokens = getViewpointTokenEntries(viewpoint && viewpoint[fieldName], fieldName, invalidEntryCode, diagnostics, viewpoint);
  var allowed = new Set(allowedValues);

  tokens.forEach(function(token) {
    if (!allowed.has(token)) {
      addDiagnostic(diagnostics, 'error', unsupportedCode, token + ' is not a supported ArchiMate viewpoint ' + fieldName + ' value.', {
        viewpointId: getConceptId(viewpoint),
        field: fieldName,
        value: token
      });
...
function getViewpointTokenEntries(value, fieldName, invalidEntryCode, diagnostics, viewpoint) {
  ...
  if (!Array.isArray(value)) {
    addDiagnostic(diagnostics, 'error', invalidEntryCode, 'Viewpoint ' + fieldName + ' entries require string values.', {
      viewpointId: getConceptId(viewpoint),
      field: fieldName,
  ...
  return value.reduce(function(tokens, entry) {
    if (typeof entry === 'string') {
      return tokens.concat(entry.split(/\s+/).filter(Boolean));
    }
...
function getModelViewpoints(model) {
  ...
  var viewpointsNode = getModelViewpointsNode(views);

  if (viewpointsNode && Array.isArray(viewpointsNode.viewpoints)) {
    return viewpointsNode.viewpoints.filter(isObject);
  }
...
function getModelViewpointsNode(views) {
  if (!isObject(views)) {
    return null;
  }

  if (Object.prototype.hasOwnProperty.call(views, 'viewpointsNode')) {
    return views.viewpointsNode;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(views, 'viewpoints')) {
    return {
      viewpoints: views.viewpoints
    };
  }

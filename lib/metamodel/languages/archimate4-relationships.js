...
var VALID_ARCHIMATE4_ELEMENT_TYPES = new Set(archimate4Profile.elements.map(function(element) {
  return element.type;
}));
...
export function setArchimate4RelationshipProfile(profile, options) {
  ...
  RELATIONSHIPS = normalizeRelationshipProfile(profile, VALID_ARCHIMATE4_ELEMENT_TYPES, loadOptions);
...
function buildFallbackRelationships() {
  ...
  FULL_ELEMENT_MAP.forEach(function(value, sourceType) {
    ...
    if (!VALID_ARCHIMATE4_ELEMENT_TYPES.has(source) || !value.relationshipMap) {
      return;
    }
    ...
    value.relationshipMap.forEach(function(relationships, targetType) {
      ...
      if (!VALID_ARCHIMATE4_ELEMENT_TYPES.has(target)) {
        return;
      }

...
* Imported and edited Access, Association, and Influence relationship options use explicit `accessType`, `isDirected`, and `modifier` properties while retaining `typeOption` compatibility. The relationship popup exposes explicit Access type actions for `None`, `Read`, `Write`, and `ReadWrite`.

...
//    'shape.delete'
    ...
    // exclude 'Note' 
    if (event.context.shape.type === 'archimate:Note') {
      return;
    }
    updateParent(event);
...
ArchimateUpdater.prototype.updateColors = function(shape) {
  ...
  if (!fillColor) {
    var color;
    /*if (isBaseElementType(shape.type)) {
      color = COLOR_LAYER_MAP.get(shape.layer);
    } else {
      color = DEFAULT_NOTE_COLOR_RGBA;
    }*/

    if (is(node, 'archimate:Node')) {
      color = COLOR_LAYER_MAP.get(shape.layer);
    } else if (is(node, 'archimate:Note')) {
      color = DEFAULT_NOTE_COLOR_RGBA;
    }
    fillColor = this.getModdleElement('archimate:FillColor', color);

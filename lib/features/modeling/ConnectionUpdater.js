...
import { ARCHIMATE_CONNECTION, ARCHIMATE_NODE, ARCHIMATE_RELATIONSHIP, ARCHIMATE_RELATIONSHIPS, RELATIONSHIP_ACCESS, RELATIONSHIP_ASSOCIATION, RELATIONSHIP_INFLUENCE } from '../../metamodel/Concept';
...
export default function ConnectionUpdater(
    ...
    translate, canvas, languageProfile) {
  ...
  function deleteConnection(context) {
    var connection = context.connection,
        ...
        viewElements = getViewElements(context.parent.businessObject);
...
ConnectionUpdater.prototype.createConnection = function(connection) {
  ...
  var viewElement = connection.businessObject,
      ...
      viewElements = getViewElements(view);
...
ConnectionUpdater.prototype.updateConnectionObjects = function(context) {
  ...
  if (connection.businessObject.type === CONNECTION_RELATIONSHIP) {
    ...
    if (isRelationshipBaseType(connection.type, RELATIONSHIP_INFLUENCE, profile)) {
      setOptionalRelationshipProperty(relationship, 'modifier', getRelationshipOptionValue(connection, 'modifier'));
    }

    if (isRelationshipBaseType(connection.type, RELATIONSHIP_ACCESS, profile)) {
      setOptionalRelationshipProperty(relationship, 'accessType', getRelationshipOptionValue(connection, 'accessType'));
    }

    if (isRelationshipBaseType(connection.type, RELATIONSHIP_ASSOCIATION, profile)) {
      setOptionalRelationshipProperty(relationship, 'isDirected', getRelationshipOptionValue(connection, 'isDirected'));
    }

    if (canApplyRelationshipMultiplicity(connection)) {
      var sourceMultiplicity = normalizeRelationshipMultiplicity(connection.sourceMultiplicity),
          targetMultiplicity = normalizeRelationshipMultiplicity(connection.targetMultiplicity);

      if (sourceMultiplicity) {
        relationship.sourceMultiplicity = sourceMultiplicity;
      }

      if (targetMultiplicity) {
        relationship.targetMultiplicity = targetMultiplicity;
      }
    }
  ...
  var viewElements = getViewElements(view);
  if (!viewElements.find(x => x.id === connection.businessObject.id)) {
    viewElements.push(connection.businessObject);
  }
...
function setOptionalRelationshipProperty(target, property, value) {

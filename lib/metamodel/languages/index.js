...
const ARCHIMATE4_MODEL_VALIDATION_CHECK_IDS = [
  ...
  'viewpoint-token-field-structure',
  ...
  'view-connection-waypoint-geometry',
  'view-style-structure',
...
export function getArchimate4ImplementationStatus(options) {
  options = options || {};
  ...
  var w262ChangeCoverage = summarizeW262ChangeCoverage(
    conformance.w262ChangeCoverageCatalog || {},
    conformance.w262ChangeCoverage || [],
  ...
  var status = {
    ...
    w262ChangeCoverage: w262ChangeCoverage,
  ...
  status.implementationCompletion = summarizeImplementationCompletionStatus(status, {
    ...
    sourceRunlogPath: options.implementationCompletionRunlogPath ||
      ARCHIMATE4_IMPLEMENTATION_COMPLETION_RUNLOG_PATH
  });

  return status;
}

export function getArchimate4ConformanceReport(implementationStatus) {
  var status = implementationStatus || getArchimate4ImplementationStatus();
  var readiness = status.conformanceReadiness || {};
  var remainingGaps = status.remainingGaps || {};
  var sourceCoverage = status.sourceCoverage || {};
  var sourceItems = sourceCoverage.items || {};
  var conformanceRequirements = status.conformanceRequirements || {};
  var requirements = conformanceRequirements.items || [];
  var gaps = remainingGaps.items || [];
  var gapsById = new Map(gaps.map(function(gap) {
    return [ gap.id, gap ];
  }));
  var requirementsByBlocker = new Map();

  requirements.forEach(function(requirement) {
    if (requirement.externalBlocker) {
      requirementsByBlocker.set(requirement.externalBlocker, requirement);
    }
  });

  var missingRequiredSources = readiness.missingRequiredSources || [];
  var missingCompanionSources = readiness.missingCompanionSources || [];
  var blockerIds = readiness.blockers || [];
  var requiredBeforeClaimByBlocker = readiness.requiredBeforeClaimByBlocker || {};
  var blockers = blockerIds.map(function(blockerId) {
    var gap = gapsById.get(blockerId) || {};
    var requirement = requirementsByBlocker.get(blockerId) || {};
    var sourceId = gap.sourceId || findSourceIdForExternalBlocker(sourceItems, blockerId);
    var source = sourceItems[sourceId] || {};

    return {
      id: blockerId,
      status: gap.status || source.status || 'external-source-dependent',
      sourceId: sourceId,
      requirementId: gap.requirementId || requirement.id,
      requiredBeforeClaim: requiredBeforeClaimByBlocker[blockerId] || '',
      missingRequiredSource: missingRequiredSources.indexOf(sourceId) !== -1,
      sourceStatus: source.status,
      sourceRunlogPath: getSourceRunlogPath(source),
      officialConformanceBlocker: true
    };
  });
  var companionGaps = gaps.filter(function(gap) {
    return gap.companion === true;
  }).map(function(gap) {
    var source = sourceItems[gap.sourceId] || {};

    return {
      id: gap.id,
      status: gap.status,
      sourceId: gap.sourceId,
      missingCompanionSource: missingCompanionSources.indexOf(gap.sourceId) !== -1,
      sourceStatus: source.status,
      sourceRunlogPath: getSourceRunlogPath(source),
      officialConformanceBlocker: gap.officialConformanceBlocker === true
    };
  });
  ...
  return {
    ...
    status: officialConformanceClaimable ? 'claimable' : 'blocked',
    reason: readiness.reason,
    officialConformanceClaimable: officialConformanceClaimable,
    externalBoundaryRetained: !officialConformanceClaimable,
    blockerIds: blockerIds,
    blockers: blockers,
    missingRequiredSources: missingRequiredSources,
    requiredBeforeClaim: readiness.requiredBeforeClaim || [],
    requiredBeforeClaimBlockerIds: readiness.requiredBeforeClaimBlockerIds || [],
    companionGapIds: companionGaps.map(function(gap) {
      return gap.id;
    }),
    companionGaps: companionGaps,
    missingCompanionSources: missingCompanionSources,
    remainingGapIds: remainingGaps.unresolvedIds || remainingGaps.actualIds || [],
    officialConformanceGapIds: remainingGaps.officialConformanceGapIds || [],
    implementedShallCount: readiness.implementedShallCount || 0,
    externalBlockedShallCount: readiness.externalBlockedShallCount || 0,
    statusRunlogPath: status.implementationCompletion && status.implementationCompletion.sourceRunlogPath
  };
}

function findSourceIdForExternalBlocker(sourceItems, blockerId) {
  var sourceIds = Object.keys(sourceItems || {});

  for (const sourceId of sourceIds) {
    if (sourceItems[sourceId] && sourceItems[sourceId].externalBlocker === blockerId) {
      return sourceId;
    }
  }
...
function getSourceRunlogPath(source) {
  return source.lastRunlogPath ||
    source.lastPublicationPageRunlogPath ||
    source.localLicenseBoundaryRunlogPath ||
    source.lastPictogramAuditRunlogPath ||
    source.localPdfInventoryRunlogPath ||
    source.localCandidateInventoryRunlogPath ||
    source.sourceRunlogPath;
}
...
function summarizeW262ChangeCoverage(catalog, topics, implementationStatusKeyIds) {
  var items = cloneProfile(topics || []);
  var expectedIds = catalog.expectedIds || [];
  var actualIds = items.map(function(topic) {
    return topic.id;
  });
  ...
  var missingPageEvidenceIds = items.filter(function(topic) {
    return !Array.isArray(topic.pageNumbers) || topic.pageNumbers.length === 0;
  }).map(function(topic) {
    return topic.id;
  });
  var statusKeyReferenceIds = [];

  items.forEach(function(topic) {
    (topic.statusKeys || []).forEach(function(statusKey) {
      appendUnique(statusKeyReferenceIds, statusKey);
    });
  });

  var missingStatusKeyReferenceIds = statusKeyReferenceIds.filter(function(statusKey) {
    return !implementationStatusKeyIdSet.has(statusKey);
  });
  var informativeIds = items.filter(function(topic) {
    return topic.classification === 'informative-rationale';
  }).map(function(topic) {
    return topic.id;
  });
  var implementedIds = items.filter(function(topic) {
    return topic.classification === 'implemented';
  }).map(function(topic) {
    return topic.id;
  });
  var expectedCount = catalog.expectedCount || expectedIds.length;
  ...
  return Object.assign({}, catalog, {
    ...
    statusKeyReferenceIds: statusKeyReferenceIds,
    missingPageEvidenceIds: missingPageEvidenceIds,
    missingStatusKeyReferenceIds: missingStatusKeyReferenceIds,
    complete: actualIds.length === expectedCount &&
      missingIds.length === 0 &&
      extraIds.length === 0 &&
      missingPageEvidenceIds.length === 0 &&

...
import {
  ...
  getSpecializedRelationshipTypesForProfile
  ...
  if (sourceElementType && targetElementType) {
    ...
    if (relationshipsAllowedString) {
      ...
      for (const char of relationshipsAllowedString) {
        ...
        addAllowedRelationship(relationshipsAllowed, relationship, excludedRelationType);

        if (profile) {
          getSpecializedRelationshipTypesForProfile(relationship, profile).forEach(function(specializedType) {
            addAllowedRelationship(relationshipsAllowed, specializedType, excludedRelationType);
          });
        }
      }
    }
...
function addAllowedRelationship(relationshipsAllowed, relationshipType, excludedRelationType) {
  if (relationshipType === excludedRelationType || relationshipsAllowed.indexOf(relationshipType) !== -1) {
    return;
  }

  relationshipsAllowed.push(relationshipType);
}

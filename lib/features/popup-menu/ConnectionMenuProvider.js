...
ConnectionMenuProvider.prototype._getAssociationIsDirected = function(element) {
  ...
  function setAssociationDirection(event, entry) {
    self._modeling.replaceRelationshipRef(element,
      { type: element.type,
        isDirected: entry.options.isDirected
      });
  }
  ...
  var newEntries = [
    {
      id: 'set-association-undirected',
      className: 'archimate-relation-association',
      title: translate('Undirected'),
      active: isUndirected,
      action: setAssociationDirection,
      options: {
        isDirected: false
      }
    },
    {
      id: 'set-association-directed',
...
ConnectionMenuProvider.prototype._getInfluenceModifier = function(element) {
  ...
  function setCustomModifier() {
    var value = readInfluenceModifierValue(
      translate('Influence modifier'),
      modifier,
      translate
    );
  ...
  if (profile && profile.version === '4.0') {
    entries.push({
      id: 'set-influence-modifier-custom',
      className: 'archimate-relation-influence-custom',
      title: translate('Set influence modifier'),
      label: translate('modifier ...'),
      active: Boolean(modifier),
      action: setCustomModifier
    });
  }
...
function readInfluenceModifierValue(title, currentValue, translate) {
  if (typeof window === 'undefined' || typeof window.prompt !== 'function') {
    return null;
  }

  var value = window.prompt(
    title + '\n' + translate('Use a sign or strength value'),
    currentValue || ''
  );
...
ConnectionMenuProvider.prototype._getMultiplicityEntries = function(element) {
  ...
  function setCustomSourceMultiplicity() {
    var value = readMultiplicityValue(
      translate('Source multiplicity'),
  ...
  function setCustomTargetMultiplicity() {
    var value = readMultiplicityValue(
      translate('Target multiplicity'),
  ...
  return [
    ...
    {
      id: 'set-source-multiplicity-custom',
      className: 'archimate-multiplicity-source-custom',
      title: translate('Set source multiplicity'),
      label: translate('source ...'),
      active: Boolean(element.sourceMultiplicity),
      action: setCustomSourceMultiplicity
    },
    {
    ...
    {
      id: 'set-target-multiplicity-custom',
      className: 'archimate-multiplicity-target-custom',
      title: translate('Set target multiplicity'),
      label: translate('target ...'),
      active: Boolean(element.targetMultiplicity),
      action: setCustomTargetMultiplicity
    }
  ];
};

function readMultiplicityValue(title, currentValue, translate) {
  if (typeof window === 'undefined' || typeof window.prompt !== 'function') {
    return null;
  }

  var value = window.prompt(
    title + '\n' + translate('Use a positive integer, *, or n..m'),
    currentValue || ''
  );
  ...
  return normalizeRelationshipMultiplicity(value);
}
...
ConnectionMenuProvider.prototype._getAccessType = function(element) {
  ...
  var isNone = !accessType || accessType === RELATIONSHIP_ACCESS_NONE,
      isRead = accessType === RELATIONSHIP_ACCESS_READ,
      isReadWrite = accessType === RELATIONSHIP_ACCESS_READWRITE,
      isWrite = accessType === RELATIONSHIP_ACCESS_WRITE;

  var accessTypeEntries = [
    {
      id: 'set-access-type-none',

...
export default function ArchimateRenderer(
  ...
  function marker(type, color) {
    var id = type+'-'+rendererId; //type + '-' + color + '-' + rendererId;
  ...
  function renderEmbeddedLabel(text, parentGfx, element, align, fontWeight, fontSize) {
  ...
  function renderArchimateConnection(parentGfx, connection, attrs) {
    var color = connection.lineColor && toRgbaString(connection.lineColor) || toRgbaString(BLACK_RGBA);
    attrs = assign({
          stroke: color,
  ...
  function renderArchimateShape(parentGfx, shape) {
    ...
    var align = shape.textAlign || 'center-middle',
      fontWeight = shape.fontStyle;

    renderEmbeddedLabel(text, parentGfx, shape, align, fontWeight);
  ...
  this.handlers = {
    ...
    */
    ...
    'archimate:Note': function(parentGfx, shape) {
      ...
      var figureRef = getFigureRef('archimate:Note');
    ...
    'archimate:Group': function(parentGfx, element) {
      ...
      renderEmbeddedLabel(text, parentGfx, element, align, fontWeight);
    ...
    'Access': function(parentGfx, connection) {
      ...
      if (connection.accessType === RELATIONSHIP_ACCESS_WRITE || 
          connection.accessType === RELATIONSHIP_ACCESS_READWRITE) {
        attrs.markerEnd = marker('opened-end', color);
      }

      if (connection.accessType === RELATIONSHIP_ACCESS_READ || 
          connection.accessType === RELATIONSHIP_ACCESS_READWRITE) {
...
ArchimateRenderer.prototype.canRender = function(element) {
  ...
  return isAny(element.businessObject, ['archimate:Node', 'archimate:Note', 'archimate:Connection']);
};

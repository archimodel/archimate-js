...
ConnectionMenuProvider.prototype.getEntries = function(element) {
  ...
  if (element.type !== CONNECTION_RELATIONSHIP) {
    ...
    relationshipsAllowed = getRelationshipsAllowed(element.source.type, element.target.type, element.type, profile);
  } else {
    relationshipsAllowed = getRelationshipsAllowed(element.source.type, element.target.type, null, profile);
  }
  var reverseRelationshipsAllowed = getRelationshipsAllowed(element.target.type, element.source.type, null, profile);
